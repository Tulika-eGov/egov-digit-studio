...
class CRSLoader:
    ...
    def login(self, username: str = None, password: str = None,
        ...
        try:
            login_url = f"{self.base_url}/user/oauth/token"
            login_data = {
                "username": username,
                "password": password,
                "tenantId": tenant_id,
    ...
    def load_mdms_v2(self, excel_path: str, target_tenant: str = None) -> Dict:
        """Phase 7: Load MDMS v2 schemas and data from Excel

        Args:
            excel_path: Path to "mdms-v2.xlsx" with 'schema' and 'data' sheets
                       Schema sheet: Column A = code, Column B = schema definition
                       Data sheet: Column B = payload data
            target_tenant: Target tenant ID

        Returns:
            dict: Summary of schema and data creation results
        """
        self._check_auth()

        print(f"\n{'='*60}")
        print(f"PHASE 7: MDMS V2 SCHEMAS & DATA")
        print(f"{'='*60}")
        print(f"File: {os.path.basename(excel_path)}")

        tenant = target_tenant or self.tenant_id
        results = {'schemas': None, 'data': None}

        try:
            # Import pandas here to avoid dependency issues
            import pandas as pd
            
            # 1. Read and create schemas
            print(f"\n[1/2] Creating MDMS v2 schemas...")
            schemas = self._read_mdms_v2_schemas(excel_path)
            
            if schemas:
                print(f"   Found {len(schemas)} schema definitions")
                results['schemas'] = self._create_mdms_v2_schemas(schemas, tenant)
            else:
                print("   No schemas found in Excel")

            # 2. Read and create data
            print(f"\n[2/2] Creating MDMS v2 data...")
            data_payloads = self._read_mdms_v2_data(excel_path)
            
            if data_payloads:
                print(f"   Found {len(data_payloads)} data payloads")
                results['data'] = self._create_mdms_v2_data(data_payloads, tenant)
            else:
                print("   No data found in Excel")

        except Exception as e:
            print(f"   ERROR: Failed to process MDMS v2 Excel: {e}")
            import traceback
            traceback.print_exc()
            return {'schemas': {'created': 0, 'failed': 1, 'error': str(e)}, 
                   'data': {'created': 0, 'failed': 1, 'error': str(e)}}

        self._print_summary("MDMS v2", results)
        return results

    def _read_mdms_v2_schemas(self, excel_path: str) -> list:
        """Read schema definitions from the 'schema' sheet
        
        Args:
            excel_path: Path to Excel file
            
        Returns:
            list: Schema definitions with code and definition
        """
        import pandas as pd
        
        try:
            # Read schema sheet
            df = pd.read_excel(excel_path, sheet_name='schema')
            schemas = []
            
            for _, row in df.iterrows():
                code = row.iloc[0] if pd.notna(row.iloc[0]) else None
                definition = row.iloc[1] if pd.notna(row.iloc[1]) else None
                ...
                if code and definition:
                    ...
                    if isinstance(definition, str):
                        try:
                            ...
                            print(f"   WARNING: Invalid JSON for schema '{code}', using as string")
    ...
    def _read_mdms_v2_data(self, excel_path: str) -> list:
        """Read data payloads from the 'data' sheet
        
        Args:
            excel_path: Path to Excel file
            
        Returns:
            list: Data payloads with schema code and data
        """
        import pandas as pd
        ...
        try:
            ...
            for _, row in df.iterrows():
                # Column A should have schema code, Column B should have payload
                schema_code = row.iloc[0] if pd.notna(row.iloc[0]) else None
                payload = row.iloc[1] if pd.notna(row.iloc[1]) else None
                
                if schema_code and payload:
                    ...
                    if isinstance(payload, str):
                        try:
                            ...
                            print(f"   WARNING: Invalid JSON for data of schema '{schema_code}', using as string")
                    
                    data_payloads.append({
                        'schema_code': str(schema_code),
                        'data': payload
                    })
    ...
    def _create_mdms_v2_schemas(self, schemas: list, tenant: str) -> Dict:
        """Create MDMS v2 schemas using the schema creation API
            ...
            try:
                ...
                schema_payload = {
                    ...
                    "SchemaDefinition": {
                        ...
                        "tenantId": tenant,
                        "code": schema['code'],
                        "description": f"Schema for {schema['code']}",
                        "definition": schema['definition'],
                        "isActive": True,
                        "auditDetails": {
                            "createdBy": self.user_info.get('id', 'system'),
                            "lastModifiedBy": self.user_info.get('id', 'system'),
                            "createdTime": int(pd.Timestamp.now().timestamp() * 1000),
                            "lastModifiedTime": int(pd.Timestamp.now().timestamp() * 1000)
                        }
                    }
                }
                
                # Create schema via MDMS v2 API
                response = requests.post(
                    f"{self.base_url}/mdms-v2/schema/v1/_create",
                    json=schema_payload,
                ...
                if response.ok:
                    created += 1
                    print(f"   ✅ Created schema: {schema['code']}")
                else:
                    failed += 1
                    print(f"   ❌ Failed to create schema '{schema['code']}': {response.status_code}")
                    try:
                        error = response.json()
                        if "already exists" in str(error).lower():
                            print(f"      (Schema may already exist)")
                            created += 1  # Count as success if already exists
                            failed -= 1
                    except:
                ...
                failed += 1
                print(f"   ❌ Error creating schema '{schema['code']}': {str(e)}")
        
        return {'created': created, 'failed': failed}

    def _create_mdms_v2_data(self, data_payloads: list, tenant: str) -> Dict:
        """Create MDMS v2 data using the data creation API
        
        Args:
            data_payloads: List of data payloads
            tenant: Target tenant
        ...
        for payload_item in data_payloads:
            try:
                # Build data creation payload
                data_payload = {
                    "RequestInfo": {
                        "apiId": "Rainmaker",
                        "authToken": self.auth_token,
                    ...
                    "Mdms": {
                        "tenantId": tenant,
                        "schemaCode": payload_item['schema_code'],
                        "data": payload_item['data']
                    }
                }
                
                # Create data via MDMS v2 API
                response = requests.post(
                    f"{self.base_url}/egov-mdms-service/v2/_create/{payload_item['schema_code']}",
                    json=data_payload,
                    headers={"Content-Type": "application/json"}
                )
                ...
                if response.ok:
                    created += 1
                    print(f"   ✅ Created data for: {payload_item['schema_code']}")
                else:
                    failed += 1
                    print(f"   ❌ Failed to create data for '{payload_item['schema_code']}': {response.status_code}")
                    try:
                        error = response.json()
                        print(f"      Error: {error}")
                    except:
                ...
                failed += 1
                print(f"   ❌ Error creating data for '{payload_item['schema_code']}': {str(e)}")
    ...
    def _read_mdms_v2_schemas(self, excel_path: str) -> list:
        ...
        try:
            ...
            if 'schema' not in wb.sheetnames:
                print("   'schema' sheet not found in Excel file")
                return []
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                ...
                definition = row[1] if row[1] else None
    ...
    def _read_mdms_v2_data(self, excel_path: str) -> list:
        ...
        try:
            ...
            if 'data' not in wb.sheetnames:
                print("   'data' sheet not found in Excel file")
                return []
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                # Column A should have schema code, Column B should have payload
                schema_code = row[0] if row[0] else None
                payload = row[1] if row[1] else None
            ...
            try:
                ...
                schema_payload = {
                    ...
                    "SchemaDefinition": {
                        ...
                        "isActive": True,
                        "auditDetails": {
                            "createdBy": self._user_info.get('id', 'system'),
                            "lastModifiedBy": self._user_info.get('id', 'system'),
                            "createdTime": int(pd.Timestamp.now().timestamp() * 1000),
                            "lastModifiedTime": int(pd.Timestamp.now().timestamp() * 1000)
                        }
                    }
